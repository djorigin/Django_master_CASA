...
class MaintenanceRecord(models.Model):
    ...
    def clean(self):
        ...
        if (self.maintenance_type.licensed_engineer_required and 
            self.status == 'completed' and not self.supervised_by):
            raise ValidationError(
                f"{self.maintenance_type.name} requires supervision by licensed engineer"
            )
        
        # CASA form completion check
        if (self.maintenance_type.casa_form_required and 
            self.status == 'completed' and not self.casa_form_completed):
            raise ValidationError("CASA form completion required for this maintenance type")
        
        # Post-maintenance hours validation
        if (self.post_maintenance_hours and 
            self.post_maintenance_hours < self.pre_maintenance_hours):
            raise ValidationError("Post-maintenance hours cannot be less than pre-maintenance hours")
