...
from .forms import (
    CustomUserForm, StaffProfileForm, PilotProfileForm, 
    ClientProfileForm, OperatorCertificateForm
)
...
def accounts_dashboard(request):
    ...
    context = {
        ...
        'available_pilots': PilotProfile.objects.filter(availability_status='available').count(),
        'active_clients': ClientProfile.objects.filter(status='active').count(),
        'active_certificates': OperatorCertificate.objects.filter(status='active').count(),
...
def staff_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            staff = form.save()
            messages.success(request, f'Staff profile for {staff.user.get_full_name()} created successfully.')
...
def staff_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'Staff profile for {staff.user.get_full_name()} updated successfully.')
            return redirect('accounts:staff_detail', pk=staff.pk)
    else:
...
def pilot_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            pilot = form.save()
            messages.success(request, f'Pilot profile for {pilot.user.get_full_name()} created successfully.')
...
def pilot_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'Pilot profile for {pilot.user.get_full_name()} updated successfully.')
            return redirect('accounts:pilot_detail', pk=pilot.pk)
    else:
...
def client_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            client = form.save()
            messages.success(request, f'Client profile for {client.user.get_full_name()} created successfully.')
...
def client_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'Client profile for {client.user.get_full_name()} updated successfully.')
            return redirect('accounts:client_detail', pk=client.pk)
    else:
...
def certificate_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Operator certificate {certificate.reoc_number} created successfully.')
...
def certificate_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'Operator certificate {certificate.reoc_number} updated successfully.')
            return redirect('accounts:certificate_detail', pk=certificate.pk)
    else:
