...
class AircraftListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['operational_aircraft'] = sum(1 for aircraft in queryset if aircraft.is_operational)
        context['pending_maintenance'] = sum(1 for aircraft in queryset if aircraft.maintenance_status in ['due_soon', 'overdue'])
...
def aircraft_compliance_dashboard(request):
    ...
    stats = {
        ...
        'maintenance_due': sum(1 for a in aircraft_qs if a.maintenance_status == 'due_soon'),
        'maintenance_overdue': sum(1 for a in aircraft_qs if a.maintenance_status == 'overdue'),
        'airworthiness_expiring': sum(
            1 for a in aircraft_qs 
            if a.airworthiness_valid_until and 
            (a.airworthiness_valid_until - timezone.now().date()).days <= 30
        )
    }
