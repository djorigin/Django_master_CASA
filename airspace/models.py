...
class OperationalArea(models.Model):
    ...
    def clean(self):
        ...
        if (self.floor_altitude_amsl and self.ceiling_altitude_amsl and 
            self.floor_altitude_amsl >= self.ceiling_altitude_amsl):
            raise ValidationError("Floor altitude must be below ceiling altitude")
        
        if (self.floor_height_agl and self.ceiling_height_agl and 
            self.floor_height_agl >= self.ceiling_height_agl):
        ...
        if self.boundary_geojson:
            try:
                ...
                if not isinstance(self.boundary_geojson, dict):
                    raise ValidationError("Boundary GeoJSON must be a valid JSON object")
